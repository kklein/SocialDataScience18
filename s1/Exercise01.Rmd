---
title: "Social Data Science R Crash Course: How tall are we?"
...
- Reading data
- Data frames
- Plotting and statistics: How tall are we?
...
**1.1) Create a vector V with the even numbers between 1 and 10. Show its content.**
```{r}
#Your code here
```

**1.2) Show its third entry and test if it is larger than 3.**
```{r}
#Your code here
```
...
You can access a single column by its name with the $ operator, for example here we get the first 3 heights
```{r}
SurveyDF$Height[1:3]
```
...
set.seed(23-2-2017)
# Generate 1000000 random numbers from normal distribution
RandomNum <- rnorm(1000000, mean=0, sd=1)
...
- And a great and interactive tutorial can be found in codeschool: <http://tryr.codeschool.com/>